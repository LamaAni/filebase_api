...
class StratisOAuth2ProviderSession {
  ...
  async update() {
    ...
    if (this.introspect_url == null) return false

    if (this.needs_access_token_validation()) {
      this.params.token_info = await this.provider.get_token_info(
        this.access_token
      )
...
class StratisOAuth2Provider {
  ...
  async svc_token(req, res, next, { refresh_token = null, auto_id = true }) {
    ...
    const info = await this.get_token_from_refresh_token(refresh_token)
    if (auto_id) {
