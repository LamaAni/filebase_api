...
const fileapi_core_source = fs.readFileSync(path.join(__dirname, 'fileapi.core.js'), 'utf-8')
...
function as_api_method(name, val) {
    return new FileApiCodeObject(val, 'REMOTE_METHOD', name)
}

function as_template_arg(name, val) {
...
class FileApiRequestEnvironment {
    ...
    get request_handlers() {
        ...
        if (this.__request_handlers == null)
            this.__request_handlers = this.env_objects.filter((o) => o.type == 'REQUEST_HANDLER')
    ...
    async check(info) {
        ...
        for (let cur of current_mtimes)
            for (let last of this.last_modification_timestamps) if (cur > last) return true
    ...
    async load_request(api, info) {
        ...
        this.last_modification_timestamps = await this.__get_modification_timestamps(info)
        ...
        this.content_type =
            mime.lookup(info.filepath, 'html/txt') + (charset ? '; charset=' + charset : '')
    }
}
...
class FileApi extends events.EventEmitter {
    constructor({
        ...
        assert(
            api_methods == null || Object.values(api_methods).every((v) => typeof v == 'function'),
        ...
        Object.keys(api_methods).forEach((k) =>
            this.env_objects.push(new FileApiCodeObject(api_methods[k], 'REMOTE_METHOD', k)),
        )
        Object.keys(ejs_environment).forEach((k) =>
            this.env_objects.push(new FileApiCodeObject(ejs_environment[k], 'TEMPLATE_ARG', k)),
        )

        this.__core_env_objects = [
            as_template_arg('render_file_api_script_tag', FileApi.render_file_api_script_tag),
            as_api_method('render_file_api_script', FileApi.render_file_api_script),
            as_template_arg('file_api', this),
        ]
    ...
    async _get_request_info(src, req) {
        ...
        const codefilepath = path.join(
            ...
            [path.basename(filepath).split('.').slice(0, -1).join('.'), this.codefile_postfix].join(
                '.',
            ),
        )
    ...
    async _handle_api_request(info, env, req, rsp, next) {
        ...
        if (name == null || env.api_methods[name] == null) return rsp.sendStatus(404)
        let return_val = await this._invoke_api_method(info, env, req, rsp, name, api_call_args)
    ...
    async _handle_file_request(info, env, req, rsp, next) {
        const is_template = this.template_extensions.some((ext) => info.filepath.endsWith(ext))
