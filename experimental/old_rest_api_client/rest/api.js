...
class RestApiRequest extends RestApiEventEmitter {
  ...
  constructor(
    ...
    this.ignore_errors_event_name =
      RestApiRequest_EVENT_NAMES.ignore_errors_event_name
  }

  static __generate_next_id() {
    global._rest_api_request_id =
      (global._rest_api_request_id == null ? -1 : global._rest_api_request_id) +
      1
...
class RestApi extends RestApiEventEmitter {
  ...
  async __send_pending_requests_loop() {
    while (this._pending_requests.length > 0) {
      ...
      await Promise.race(
        Object.values(this._active_requests).map((rq) => rq.promise)
      )
    }
  ...
  async wait_until_complete(requests, throw_errors = true) {
    ...
    const handler = new RestApiEventEmitter()
    const complete_event_names = Array.from(
      new Set(requests.map((r) => r.complete_event_name))
    )
    const error_event_names = Array.from(
      new Set(requests.map((r) => r.error_event_name))
    )
    ...
    if (throw_errors)
      for (let name of error_event_names) handler.on(name, on_error)
...
if (require.main == module) {
  ...
  ;(async () => {
    const requests = new Array(100).fill(0).map(() => {
      ...
      rq.on(rq.complete_event_name, () =>
        log.info('Completed request ' + rq.id)
      )
